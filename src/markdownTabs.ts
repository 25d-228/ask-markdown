...
const collectMarkdownTargets = async (
	ourViewType: string,
	diag: vscode.OutputChannel,
): Promise<MarkdownTarget[]> => {
...
export const resolveTargetMarkdown = async (
	ourViewType: string,
	diag: vscode.OutputChannel,
): Promise<MarkdownTarget | undefined> => {
	const targets = await collectMarkdownTargets(ourViewType, diag);
